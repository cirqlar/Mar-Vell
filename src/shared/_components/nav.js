...
function More({ name, links, open, close }) {
  const [isOpen, setIsOpen] = useState(false);
  ...
  return (
    <li ref={thisLink} className={styles.more}
      onBlur={() => {
        tabable(thisLink, thisClose);
      }}
    >
      ...
      >{name}</button>
      <div className={cn({
        [styles.moreOpen]: isOpen,
      })}>
        <ul className={styles.morePrimary}>
